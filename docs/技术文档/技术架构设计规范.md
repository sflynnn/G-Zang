...
> **最后更新**：2026-03-27
> **维护人员**：后端架构师 / 前端架构师
> **关联规则**：`backend.mdc`、`security.mdc`、`devops.mdc`
...
| 策略 | 适用场景 | 实现方式 |
|------|---------|---------|
| URL 路径版本 | 公开 API、主版本迭代 | `/api/v1/`、`/api/v2/` |
| Header 版本 | 微服务内部接口 | `Accept: application/vnd.gzang.v1+json` |
| Query 参数版本 | 过渡期兼容 | `?version=2`（不推荐） |

> **规范**：主版本不兼容时使用 URL 路径版本（如 `/api/v2/`），小版本兼容时在响应头中声明支持的版本。

### 10.2 版本演进原则

| 原则 | 说明 |
|------|------|
| 渐进式废弃 | 在响应头中声明废弃字段 `Deprecation: true`，给予 3 个月迁移期 |
| 响应头扩展 | 新增字段使用 `X-` 前缀或独立响应头，不破坏现有结构 |
| 向后兼容 | 数据库字段变更通过添加新字段而非修改现有字段 |
| API 文档同步 | 每个版本必须有对应的 API 文档，且明确说明差异 |

---

## 11. 分布式事务

> 当跨多个服务或数据库操作需要保证一致性时使用。

### 11.1 事务模式选型

| 模式 | 适用场景 | 实现方案 |
|------|---------|---------|
| TCC (Try-Confirm-Cancel) | 强一致性、高并发 | Seata TCC Mode |
| Saga | 长事务、跨系统 | Seata Saga Mode |
| 最大努力通知 | 非核心流程 | 本地消息表 + MQ |
| 最终一致性 | 大多数业务场景 | 本地消息表 |
...
| 组件 | 适用场景 | 特点 |
|------|---------|------|
| RabbitMQ | 中小规模、灵活路由 | 延迟低、路由灵活 |
| Apache Kafka | 大数据、日志、高吞吐 | 高吞吐、持久化、分区 |
| RocketMQ | 电商、金融事务 | 事务消息、顺序消息 |

### 12.2 可靠性保障

| 机制 | 实现 |
|------|------|
| 生产者确认 | Publisher Confirms（RabbitMQ） |
| 消息持久化 | 开启 `durable=true` + 磁盘存储 |
| 消费者确认 | Manual ACK，处理成功才确认 |
| 死信队列 | `x-dead-letter-exchange` + `x-dead-letter-routing-key` |
| 消息幂等 | 业务唯一键 + Redis/MongoDB 去重 |
...
// ✅ 消费者幂等处理
@RabbitListener(queues = "transaction.notify")
public void handleTransaction(TransactionMessage msg, Channel channel,
                               @Header(AmqpHeaders.DELIVERY_TAG) long tag) {
    String key = "msg:transaction:" + msg.getId();
...
**最后更新**：2026-03-27
**维护人员**：后端架构师 / 前端架构师
**关联规则**：`backend.mdc`、`security.mdc`、`devops.mdc`、`ui-design.mdc`
